...
public:
    ...
    static void Log(LogLevel level, std::string_view fmt, Args &&... args) {
        ...
        switch (level) {
            case LogLevel::Info: log_color = LOG_COLOR_CYAN;
                level_str = "INFO";
                break;
            case LogLevel::Warning: log_color = LOG_COLOR_YELLOW;
                level_str = "WARNING";
                break;
            case LogLevel::Error: log_color = LOG_COLOR_RED;
                level_str = "ERROR";
                break;
            default: log_color = LOG_COLOR_RESET;
                level_str = "LOG";
                break;
        }
...
#else
    template<typename... Args>
    static void Info(std::format_string<Args...>, Args &&...) {
    }
    template<typename... Args>
    static void Warning(std::format_string<Args...>, Args &&...) {
    }
    template<typename... Args>
    static void Error(std::format_string<Args...>, Args &&...) {
    }
