...
ui::window::window(const char *title)
    : open(ImGui::Begin(title))
{
    if (!open) {
        Debug::log(LogLevel::WARNING, "Failed to create window");;
    }
}
...
void MelonImGui::Init() {
    ...
    const uint32_t swapchainImageCount = static_cast<uint32_t>(_device->GetSwapchainImages().size());
...
void MelonImGui::immediate_submit(std::function<void(VkCommandBuffer cmd)> func) {
    _device->ImmediateSubmit(std::move(func));
}
...
void MelonImGui::draw_debug_ui() {
    if (const ui::window debug("Backgrounds"); debug) {
...
void MelonImGui::init_imgui_styles()
{
...
void MelonImGui::draw_app_top_menu() {
